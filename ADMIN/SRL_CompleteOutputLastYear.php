...
header('Content-Disposition: attachment; filename="' . $deptLngName . 'RefLetter' . date("Y-m-d", strtotime("-1 years")) . 'ToPresent.csv"');
...
$sql = "SELECT Letter.refLetter_id,Letter.refLetter_dueDate,Letter.refLetter_FKstudent_uniqname,
	...
	WHERE Letter.created > '$oneYearAgo'
	ORDER BY Letter.refLetter_FKstudent_uniqname ASC,  Letter.refLetter_institute_name ASC";
// fetch the data
$rowsOutputFile = mysqli_query($db, $sql);

// loop over the rows, outputting them
while ($row = mysqli_fetch_assoc($rowsOutputFile)) fputcsv($output, $row);

mysqli_free_result($rowsOutputFile);
mysqli_close($db);